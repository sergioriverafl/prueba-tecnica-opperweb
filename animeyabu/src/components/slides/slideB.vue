<template>

    <div class="flex items-center justify-center">
        <div class="rounded-full w-80 h-80 bg-secondary">
            <img src="./../../assets/img/img-slide-2.png" alt="" class="h-96 pt-10 hover:scale-125">
        </div>

    </div>

    <div class="flex flex-col gap-8 items-center">
        <h1 class="font-bold text-5xl text-white">anime<span class="text-primary">Kyojuro.</span></h1>
        <p class="text-white text-center w-80">
            Ver anime en línea en HD, subtitulado o doblado,
            en tu celular o computadora.
            ¡Animeyabu, tu portal de anime en línea!
        </p>
    </div>

</template>

<script>

    export default {
        name: 'slideB',
        data() {
            return {
            }
        },
        mounted: function () {
        },
        methods: {
        }
    }
</script>